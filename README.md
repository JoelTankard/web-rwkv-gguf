...
-   **GGUF format support** - Load models in llama.cpp's GGUF format.
...
It _does not_ provide the following:
...
-   Samplers, though in the examples a basic nucleus sampler is implemented, this is _not_ included in the library itself.
...
-   To specify the location of your model (SafeTensors or GGUF), use
...
_You must download the model and put in `assets/models` before running if you are building from source._
...
GGUF is the model format used by [llama.cpp](https://github.com/ggerganov/llama.cpp). You can convert RWKV models to GGUF using the provided [`convert_hf_to_gguf.py`](assets/scripts/convert_hf_to_gguf.py) script:
...
-   `f16` - 16-bit float (recommended for quality)
-   `bf16` - bfloat16
-   `f32` - 32-bit float
-   `q8_0` - 8-bit quantization

**Note:** The GGUF loader currently loads F16/BF16/F32 models and applies runtime quantization. Direct loading of pre-quantized GGUF tensors (Q8_0, Q4_K) is planned for a future release to reduce peak VRAM usage.
