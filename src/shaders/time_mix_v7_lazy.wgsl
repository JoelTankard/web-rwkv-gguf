...
@group(0) @binding(0) var<uniform> shape: vec4<u32>;                    // [S, H, T]
...
#ifdef FP16
@group(0) @binding(5) var<storage, read> r: array<vec2<u32>>;           // (T, H, S)
@group(0) @binding(6) var<storage, read> w: array<vec2<u32>>;           // (T, H, S)
@group(0) @binding(7) var<storage, read> n: array<vec2<u32>>;           // (4, T, H, S)
@group(0) @binding(9) var<storage, read_write> x: array<vec2<u32>>;     // (T, H, S)
#else
@group(0) @binding(5) var<storage, read> r: array<vec4<f32>>;           // (T, H, S)
@group(0) @binding(6) var<storage, read> w: array<vec4<f32>>;           // (T, H, S)
@group(0) @binding(7) var<storage, read> n: array<vec4<f32>>;           // (4, T, H, S)
@group(0) @binding(9) var<storage, read_write> x: array<vec4<f32>>;     // (T, H, S)
#endif
...
fn time_mix(in: Input) {
    ...
    for (var t = 0u; t < num_tokens; t += 1u) {
        ...
        if index < stride && t + 1u == num_tokens {
        ...
        if index < stride {
            ...
            for (var j = 0u; j < HEAD_SIZE; j += 1u) {
                ...
                var bji = compute_index(batch, j * 4u + 1u, index);
