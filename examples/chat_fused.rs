...
use web_rwkv::{
    ...
    runtime::{
        ...
        softmax::softmax_one,
        v4, v5, v6, v7, v7_fused, Runtime, TokioRuntime,
    },
...
async fn main() -> Result<()> {
    ...
    let (info, context, (runtime, state)) = if is_gguf {
        ...
        let result: Result<(Box<dyn Runtime<Rnn>>, Box<dyn State>)> = match info.version {
            ...
            ModelVersion::V7 => {
                if cli.fused {
                    log::info!("Using v7_fused (fused token shift + layer norm)");
                    let model = builder.build_v7_fused().await?;
                    let bundle = v7_fused::Bundle::<f16>::new(model, 1);
                    let state = bundle.state();
                    let runtime = TokioRuntime::new(bundle).await;
                    Ok((Box::new(runtime), Box::new(state)))
                } else {
