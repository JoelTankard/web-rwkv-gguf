...
fn dispatch_layer<F: Float>(
    ...
    // Use fused LayerNorm + Token Shifts for single-token inference (7 dispatches → 1)
    // For multi-token prefill, use separate ops since fused shader can't handle token > 0
    if num_token == 1 {
        ops.extend([
            TensorOp::fused_token_shift_ln(
                &buffer.cursors,
                &layer.att_ln.w,
                &layer.att_ln.b,
                state.att(index)?,
        ...
        // Fall back to separate ops for multi-token prefill
        ops.extend([
            TensorOp::layer_norm(
                &layer.att_ln.w,
                &layer.att_ln.b,
...
impl<R: Reader> ModelBuilder<R> {
    pub async fn build_v7_fused(self) -> Result<Model, LoaderError> {
        let ModelBuilder {
            context,
            model,
            rescale,
